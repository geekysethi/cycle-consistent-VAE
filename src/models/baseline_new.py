...
class Decoder(nn.Module):
	def __init__(self, style_dim, class_dim):
		...
		self.deconv_model = nn.Sequential(OrderedDict([
			...
			('deconvolution_1_in', nn.InstanceNorm2d(num_features = 128, track_running_stats=True)),
...
class Prediction(nn.Module):
	def __init__(self, z_dim,out_dim):
		...
		self.fc_model = nn.Sequential(OrderedDict([
			...
			('fc_2_bn', nn.BatchNorm1d(num_features=256)),
			('LeakyRelu_2', nn.LeakyReLU(negative_slope=0.2, inplace=True)),
			('fc_3', nn.Linear(in_features=256, out_features = out_dim, bias=True))
...
if __name__ == '__main__':
	...
	encoder = Encoder(512, 64)
	decoder = Decoder(512, 64)
	...
	mu, logvar, class_latent_space = encoder(Variable(x))
