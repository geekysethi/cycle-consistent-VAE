...
class Prediction(nn.Module):
	def __init__(self, z_dim):
		...
		self.fc_model = nn.Sequential(OrderedDict([
			...
			('fc_2_bn', nn.BatchNorm1d(num_features=256)),
			('LeakyRelu_2', nn.LeakyReLU(negative_slope=0.2, inplace=True)),
			('fc_3', nn.Linear(in_features=256, out_features=z_dim, bias=True))


		]))

		# self.style_mu = nn.Linear(in_features=256, out_features=z_dim, bias=True)
		# self.style_logvar = nn.Linear(in_features=256, out_features=z_dim, bias=True)
	...
	def forward(self, z):        
		...
		# return style_embeddings_mu, style_embeddings_logvar
