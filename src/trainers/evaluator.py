...
class ResNetEvaluator:
    ...
    def save_incorrect_pairs(self, distmat, queryloader, galleryloader, 
        g_pids, q_pids, g_camids, q_camids, savefig_path):
        ...
        for i in range(m):
            ...
            for j in range(10):
                ...
                axes[j+1].imshow(img)
            fig.savefig(os.path.join(savefig_path, '%d.png' %q_pids[i]))
            plt.close(fig)

    def evaluate(self, queryloader, galleryloader, queryFliploader, galleryFliploader, 
        ranks=[1, 2, 4, 5,8, 10, 16, 20], eval_flip = True, re_ranking = False, savefig = False):
        self.model.eval()
        qf, q_pids, q_camids = [], [], []
        ...
        if savefig:
            ...
            self.save_incorrect_pairs(distmat.numpy(), queryloader, galleryloader, 
                g_pids.numpy(), q_pids.numpy(), g_camids.numpy(), q_camids.numpy(), savefig_path)
